...
export async function GET(request: NextRequest, { params }: { params: { nickname: string } }) {
    ...
    if (data) {
        ...
        if (googleData.items) {
            response.profile_picture_url = googleData.items[0].snippet.thumbnails.medium.url;
            response.subcount = googleData.items[0].statistics.subscriberCount
        }

    }
    //RETURNS EMPTY PROFILE PICTURE
